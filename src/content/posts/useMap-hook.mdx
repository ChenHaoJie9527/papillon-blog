...
4. **`useCallback` 空依赖**：`set` / `setAll` / `remove` / `reset` 的函数引用稳定，可以放进其他 Hook 的依赖数组。
...
2. **解构 action，不要依赖整个 `actions` 对象**。每次渲染都会返回新的 `{ set, setAll, remove, reset }`，对象引用会变；四个方法本身是稳定的。写成 `const { set, remove } = actions` 再放进 `useCallback` / `useEffect` 依赖。
