    ...
    <DemoContainer>
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4 text-sm">
          ...
          <div>
            <p className="text-foreground/70">
              节流后位置: <span className="text-accent">{throttledScrollY}px</span>
            </p>
          </div>
        </div>
        <p className="text-sm text-foreground/70">
    ...
    <DemoContainer>
      <div className="space-y-4">
        ...
        <div className="space-y-2 text-sm">
          ...
          <p className="text-foreground/70">
            节流后的执行次数: <span className="text-accent">{throttledClickCount}</span>
          </p>
        </div>
      </div>
    </DemoContainer>
...
export function GameControlThrottleDemo() {
  ...
  const throttledMove = useThrottle((direction: 'left' | 'right') => {
    setThrottledPosition((prev) => {
      const newPos =
        direction === 'left' ? Math.max(0, prev - 10) : Math.min(100, prev + 10)
      return newPos
    })
    ...
    <DemoContainer>
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <p className="text-xs text-foreground/70">实时位置: {playerPosition}%</p>
        </div>
        <div className="space-y-2">
          <div className="h-8 bg-foreground/10 rounded relative">
            <div
          ...
          <p className="text-xs text-foreground/70">节流位置: {throttledPosition}%</p>
        </div>
        <p className="text-sm text-foreground/70">
